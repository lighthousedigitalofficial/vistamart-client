...
const ProfileInfo = () => {
	...
	const { data: user, isLoading: customerLoading } = useGetCustomerDetailsQuery(
		userInfo.user._id,
		{
	...
	useEffect(() => {
		if (userInfo && user?.doc) {
			selectedImage(`${keys.BUCKET_URL}${user?.doc?.image}`);
		}
	}, [selectedImage, user, userInfo]);
